import { getAuth, signInWithPopup, GoogleAuthProvider, GithubAuthProvider, AuthProvider } from "firebase/auth";
import { initializeApp } from "firebase/app";
import firebaseConfig from "./firebaseConfig";

initializeApp(firebaseConfig);

const redirectToMyPageWhenLoginSuccess = async (provider: AuthProvider): Promise<void> => {
  try {
    const auth = getAuth();
    ...
    if (error.code === "auth/account-exists-with-different-credential") {
      alert(`${error.customData.email}は他の SNS と連携した既存ユーザーが登録済みです。既存ユーザーでログイン後、こちらの SNS との連携が可能です。`);
      return;
    }
